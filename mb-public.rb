...
### 設定
###
public_file_path = "public/study"
#public_file_path = "/lustre6/public/metabobank/study"
...
OptionParser.new{|opt|
	...
	opt.on('-r [release type]', 'initial or re-distribution'){|v|
		raise "usage: -r release type, ini for initial release or re for re-distribution)" unless ["ini", "re"].include?(v)
		release_type = v
	}

	# 過去に遡ったデータ公開の場合
	opt.on('-d [initial release date]', 'initial release date'){|v|
		raise "usage: -d initial release date (yyyy-mm-dd)" unless v =~ /20\d{2}-\d{2}-\d{2}/
		initial_release_date = v
	}

	# 過去に遡ったデータ公開の場合
	opt.on('-u [last update date]', 'last update date'){|v|
	...
	begin
		...
		if idf_path.empty?
			raise "usage: -i IDF file (MTBKS1.idf.txt)"
		end

		if release_type.empty?
			raise "usage: -r release type, ini for initial release or re for re-distribution)"
		end
